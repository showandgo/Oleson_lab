...
list(unique(moku_marine$ISLAND))

isl <- "Hawaii"

list(moku_marine$NAME2[moku_marine$ISLAND == isl])

mok <- list("HAMAKUA", "KOHALA" ,  "HILO"   ,  "PUNA HAW", "KONA HAW", "KAU")
...
make_df <- function(mokus, island){
  cropped <- (crop(x = bent_proj, y = extent(moku_marine[moku_marine$NAME2 == mokus,])))
  marinemoku_crop <- moku_marine[moku_marine$NAME2 == mokus,]
  moku_area <- sum(st_area(marinemoku_crop[marinemoku_crop$NAME2 == mokus,]))
...
Compile all Moku in an island

```{r}

for (i in mok){
  df <- bind_rows(df, make_df(i, isl))
}
...
```{r}
write.csv(df,paste0("C:/Extent Map proj/", isl, "_benthic.csv") )
```
